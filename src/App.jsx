...
function App() {
  ...
  useEffect(() => {
    const setupPublicDirectory = async () => {
      try {
        const hfs = new NodeHfs();
        const publicExists = await hfs.isDirectory('public');
        if (!publicExists) {
          await hfs.createDirectory('public');
        }

        const requiredFiles = ['college-logo.png', 'funny-video.mp4'];
        for (const file of requiredFiles) {
          const fileExists = await hfs.isFile(`public/${file}`);
          if (!fileExists) {
            console.warn(`Warning: Required file ${file} not found in public directory`);
          }
        }
      } catch (error) {
        console.error('Error while setting up public directory:', error);
      }
    };
  ...
  const handleSubmit = (e) => {
    ...
    // Updated USN pattern to be more flexible
